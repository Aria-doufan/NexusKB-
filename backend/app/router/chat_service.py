...
class ChatService:
    ...
    async def handle_agent_query(self, query: str, session_id: Optional[str], user_id: str) -> Tuple[str, str, list]:
        # Keep legacy non-stream callers on the same RouterGraph main path.
        result = await router_graph.invoke(query=query, session_id=session_id, user_id=user_id)
        return (
            result.get("session_id") or session_id or str(uuid.uuid4()),
            result.get("response", ""),
            result.get("steps", []),
        )

    async def handle_router_query(self, query: str, session_id: Optional[str], user_id: str) -> Dict[str, Any]:
        """处理 LangGraph Router 查询逻辑"""
        return await router_graph.invoke(query=query, session_id=session_id, user_id=user_id)
    ...
    async def handle_list_memories(self, user_id: str, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
    ...
    async def handle_reorder(self, query: str, documents: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if result["success"]:
                ranked_documents = [f"文档 {doc['document']}: {doc['similarity']:.4f}" for doc in result["documents"]]
                logger.info(f"【重排序结果】查询: {query} 排序结果: {ranked_documents}")
