...
@pytest.mark.anyio
async def test_router_graph_preserves_web_reference_sources_from_agentic_rag_steps():
    from app.agent.router_graph import RouterGraph
    ...
    class FakeAgenticRagGraph:
        async def invoke(self, query, user_id, session_id=None):
            return {
                ...
                "confidence": 0.88,
                "reason": "Needs policy source.",
                "response": "公司知识库未找到足够资料。以下为通用参考。",
                "steps": [{"tool": "agentic_rag_graph", "tool_output": {"sources": [web_source]}}],
                "error": None,
            }
    ...
    result = await router.invoke("如果公司知识库没有报销流程，给我一个通用流程参考", "user-1", "sess-web-source")

    assert result["steps"][0]["tool_output"]["sources"][0]["source_type"] == "web_reference"
    assert result["steps"][0]["tool_output"]["sources"][0]["metadata"]["reference_scope"] == "general_public_reference"
